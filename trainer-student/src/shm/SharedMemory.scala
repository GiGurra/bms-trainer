...
class SharedMemory(
    ...
    def read(ints: Array[Int], offs: Int, n: Int) {
        read(0, ints, offs, n)
    }
    
    def read(ptrOffs: Int, ints: Array[Int], offs: Int, n: Int) {
        view.foreach(_.read(ptrOffs, ints, offs, n))
    }
    ...
    def read(ptrOffs: Int, bytes: Array[Byte], offs: Int, n: Int) {
        view.foreach(_.read(ptrOffs, bytes, offs, n))
    }
